    ...
    $date_today = date("Y-m-d");
		...
		if($count >0){
			...
			//$getleads = "select l.*,u.uom, v.country_id from leads l, uoms u, lead_display_countries v, subscription_chapter sc  where l.uom_id=u.id and l.lead_type = 'Buy' and l.id=v.lead_id and l.chapter_id = '$key' and v.country_id = '$country_id' and l.posted_by != '$user_id' and sc.user_id = l.posted_by and l.status=1 and l.expiry_date>'$date_today' and l.id NOT IN (SELECT lead_id from purchased_leads where user_id = '$user_id') group by l.id ";
			  $getleads = "select l.*,u.uom, (select count(*) from myfav_hscodes where user_id='$user_id' and hscode=l.hsn_id) as hsstate, (select remind_date from lead_reminders where user_id='$user_id' and lead_id=l.id) as laterstate from leads l, uoms u,  subscription_chapter sc where l.uom_id=u.id and l.lead_type = 'Buy' and l.chapter_id = '$key' and l.country_id = '$country_id' and l.posted_by != '$user_id' and sc.user_id != l.posted_by and l.status=1 and l.expiry_date >= '$date_today' and l.id NOT IN (SELECT lead_id from purchased_leads where user_id = '$user_id') group by l.id ";
