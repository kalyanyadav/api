...
if (isset($_SERVER['HTTP_ORIGIN'])) {
	...
	$rangeDate = $yearmonth."-01";
		...
		$sqlseller="SELECT YEAR(created) AS y, MONTH(created) AS m, COUNT(DISTINCT id) as chartPoint FROM users_1 WHERE YEAR(created) > '$year' AND `user_type` ='seller' GROUP BY y, m";
		...
		$sqlbuyer="SELECT YEAR(created) AS y, MONTH(created) AS m, COUNT(DISTINCT id) as chartPoint FROM users_1 WHERE YEAR(created) > '$year' AND `user_type` ='buyer' GROUP BY y, m";
		...
		$sqlboth="SELECT YEAR(created) AS y, MONTH(created) AS m, COUNT(DISTINCT id) as chartPoint FROM users_1 WHERE YEAR(created) > '$year' AND `user_type` ='both' GROUP BY y, m";
		...
		$sqlother="SELECT YEAR(created) AS y, MONTH(created) AS m, COUNT(DISTINCT id) as chartPoint FROM users_1 WHERE YEAR(created) > '$year' AND `user_type` ='other' GROUP BY y, m";
