...
if (isset($_SERVER['HTTP_ORIGIN'])) {
	...
	if($search_type=='category'){
		...
		if($count <=0){
			...
			//print_r($cid);
			foreach($cid as $key){
			$getleads = "select * from leads where categories_id = '$key' and expiry_date >'$date_today' and lead_type='Buy'";
	...
	elseif($search_type=='chapter'){
		...
		if(mysqli_num_rows($cresult) <= 0){
			...
			foreach($chid as $key){
			$getleads = "select * from leads where chapter_id = '$key' and expiry_date >'$date_today' and lead_type='Buy'";
		...
		if(mysqli_num_rows($hresult) <= 0){
			...
			foreach($hid as $key){
			$getleads = "select * from leads where hsn_id = '$key' and expiry_date >'$date_today' and lead_type='Buy'";
