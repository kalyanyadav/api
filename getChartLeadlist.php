...
if (isset($_SERVER['HTTP_ORIGIN'])) {
	...
	$rangeDate = $yearmonth."-01";
		...
		$sqlseller="SELECT YEAR(posted_date) AS y, MONTH(posted_date) AS m, COUNT(DISTINCT id) as chartPoint FROM leads WHERE YEAR(posted_date) > '$year' AND lead_type <> '' AND lead_type='sell' GROUP BY y, m";
		...
		$sqlbuyer="SELECT YEAR(posted_date) AS y, MONTH(posted_date) AS m, COUNT(DISTINCT id) as chartPoint FROM leads WHERE YEAR(posted_date) > '$year' AND lead_type <> '' AND lead_type='buy' GROUP BY y, m";
